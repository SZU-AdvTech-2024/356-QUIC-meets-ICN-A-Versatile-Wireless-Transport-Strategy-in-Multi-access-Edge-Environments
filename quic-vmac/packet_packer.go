...
func (p *packetPacker) PackCoalescedPacket(onlyAck bool, maxPacketSize protocol.ByteCount, v protocol.VersionNumber) (*coalescedPacket, error) {
	...
	if (onlyAck && size == 0) || (!onlyAck && size < maxPacketSize-protocol.MinCoalescedPacketSize) {
		...
		if err == nil { // 1-RTT
			...
			oneRTTPayload = p.maybeGetShortHeaderPacket(oneRTTSealer, hdrLen, maxPacketSize-size, onlyAck, size == 0, v, -1)
...
func (p *packetPacker) appendPacket(buf *packetBuffer, onlyAck bool, maxPacketSize protocol.ByteCount, v protocol.VersionNumber) (shortHeaderPacket, error) {
	...
	// get the potential frames
	pl := p.maybeGetShortHeaderPacket(sealer, hdrLen, maxPacketSize, onlyAck, true, v, pn)
	if pl.length == 0 {
...
func (p *packetPacker) maybeGetAppDataPacketFor0RTT(sealer sealer, maxPacketSize protocol.ByteCount, v protocol.VersionNumber) (*wire.ExtendedHeader, payload) {
	...
	return hdr, p.maybeGetAppDataPacket(maxPayloadSize, false, false, v, -1)
}

func (p *packetPacker) maybeGetShortHeaderPacket(sealer handshake.ShortHeaderSealer, hdrLen protocol.ByteCount, maxPacketSize protocol.ByteCount, onlyAck, ackAllowed bool, v protocol.VersionNumber, pn protocol.PacketNumber) payload {
	maxPayloadSize := maxPacketSize - hdrLen - protocol.ByteCount(sealer.Overhead())
	return p.maybeGetAppDataPacket(maxPayloadSize, onlyAck, ackAllowed, v, pn)
}

func (p *packetPacker) maybeGetAppDataPacket(maxPayloadSize protocol.ByteCount, onlyAck, ackAllowed bool, v protocol.VersionNumber, pn protocol.PacketNumber) payload {
	pl := p.composeNextPacket(maxPayloadSize, onlyAck, ackAllowed, v, pn)
...
func (p *packetPacker) composeNextPacket(maxFrameSize protocol.ByteCount, onlyAck, ackAllowed bool, v protocol.VersionNumber, pn protocol.PacketNumber) payload {
	if onlyAck {
	...
	// whether it is control frames or stream frames
	...
	if hasData {
		...
		// MOC: record the offset of the frame belongs to which packet number
		if pn != -1 {
			for _, v := range pl.streamFrames {
				if IQPerspective == protocol.PerspectiveServer {
					seminars[0].offsetPnMap.Lock()
					seminars[0].offsetPnMap.m[v.Frame.Offset] = pn
					seminars[0].offsetPnMap.Unlock()
					//if _, ok := seminars[0].offsetPnMap.m[v.Frame.Offset]; !ok {
					//	seminars[0].offsetPnMap.Lock()
					//	seminars[0].offsetPnMap.m[v.Frame.Offset] = pn
					//	seminars[0].offsetPnMap.Unlock()
					//} else {
...
func (p *packetPacker) MaybePackProbePacket(encLevel protocol.EncryptionLevel, maxPacketSize protocol.ByteCount, v protocol.VersionNumber) (*coalescedPacket, error) {
	if encLevel == protocol.Encryption1RTT {
		...
		pn, pnLen := p.pnManager.PeekPacketNumber(protocol.Encryption1RTT)
		hdrLen := wire.ShortHeaderLen(connID, pnLen)
		pl := p.maybeGetAppDataPacket(maxPacketSize-protocol.ByteCount(s.Overhead())-hdrLen, false, true, v, -1)
		if pl.length == 0 {
			return nil, nil
		}
		buffer := getPacketBuffer()
