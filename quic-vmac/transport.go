...
func (t *Transport) init(isServer bool) error {
	t.initOnce.Do(func() {
		...
		if t.ConnectionIDGenerator != nil {
			...
			// generates a connection ID using cryptographic random
...
func (t *Transport) handlePacket(p receivedPacket) {
	...
	if err != nil {
		t.logger.Infof("error parsing connection ID on packet from %s: %s", p.remoteAddr, err)
		if t.Tracer != nil {
	...
	// get connection handler with connID
	if handler, ok := t.handlerMap.Get(connID); ok {
		// MOC: aggregate all the connections id to the first main connection
		//if seminars[0].ifHandshakeDone && handler != seminars[0].connections[0] {
		//	seminars[0].connections[0].handlePacket(p)
		//	return
		//}
		// if connID is empty, the default handler will be found
	...
	// only when the connection has not been established will it reach this logic
