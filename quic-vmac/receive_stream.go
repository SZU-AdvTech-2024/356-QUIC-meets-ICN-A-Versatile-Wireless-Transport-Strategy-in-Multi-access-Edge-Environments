...
func (s *receiveStream) readImpl(p []byte) (bool /*stream completed */, int, error) {
	...
	// bytesRead have an equal meaning of bytesWritten
	var bytesRead int
	var deadlineTimer *utils.Timer
	for bytesRead < len(p) {
		// s.currentFrame is nil at begin
		if s.currentFrame == nil || s.readPosInFrame >= len(s.currentFrame) {
			s.dequeueNextFrame()
		}
		// currentFrame still is nil after handle dequeueNextFrame(), means error
		...
		for {
			...
			// s.deadline is invoked by the userspace on its own initiative, normally it is zero, means read now until the data comes and no timeout
			deadline := s.deadline
			if !deadline.IsZero() {
			...
			// sometimes currentFrame has a data, break directly. If not, will execute dequeueNextFrame and next round the currentFrame will not be nil
			if s.currentFrame != nil || s.currentFrameIsLast {
				break
			}

			s.mutex.Unlock()
			// the below logic seems that wait until can read, it waits readChan with or without select{}
			if deadline.IsZero() {
				<-s.readChan
			} else {
				select {
		...
		// the p passed in this function is a null byte firstly, we fill it with frame data frame by frame
...
// dequeueNextFrame is called at the readImpl when handleStreamFrameImpl calls signalRead normally, the others that call the signalRead is unusual status like close
func (s *receiveStream) dequeueNextFrame() {
	...
	// every time in read a new frame, set the read position to 0
	s.readPosInFrame = 0
}
...
// getWindowUpdate is called every round sending a packet, every stream has a unique getWindowUpdate()
