...
type rawConn interface {
	...
	// WritePacket The size parameter is used for GSO.
...
func (h *packetHandlerMap) AddWithConnID(clientDestConnID, newConnID protocol.ConnectionID, fn func() (packetHandler, bool)) bool {
	...
	// create the new connection and handle the first packet
	conn, ok := fn()
	if !ok {
		return false
	}
...
func (h *packetHandlerMap) Retire(id protocol.ConnectionID) {
	h.logger.Infof("Retiring connection ID %s in %s.", id, h.deleteRetiredConnsAfter)
	time.AfterFunc(h.deleteRetiredConnsAfter, func() {
		h.mutex.Lock()
		delete(h.handlers, id)
		h.mutex.Unlock()
		h.logger.Infof("Removing connection ID %s after it has been retired.", id)
	})
}
...
func (h *packetHandlerMap) ReplaceWithClosed(ids []protocol.ConnectionID, pers protocol.Perspective, connClosePacket []byte) {
	...
	time.AfterFunc(h.deleteRetiredConnsAfter, func() {
		...
		h.mutex.Unlock()
		h.logger.Infof("Removing connection IDs %s for a closed connection after it has been retired.", ids)
	})
}
