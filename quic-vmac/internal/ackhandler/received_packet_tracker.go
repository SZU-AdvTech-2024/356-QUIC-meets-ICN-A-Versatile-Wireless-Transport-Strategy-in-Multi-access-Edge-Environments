...
func (h *receivedPacketTracker) ReceivedPacket(packetNumber protocol.PacketNumber, ecn protocol.ECN, rcvTime time.Time, shouldInstigateAck bool) error {
	...
	// isMissing means whether this packet have been ack (or in ack process)
...
// GetAckFrame has three different packet levels implementation, and onlyIfQueued means if this packet has ack frame only
func (h *receivedPacketTracker) GetAckFrame(onlyIfQueued bool) *wire.AckFrame {
	...
	now := time.Now()
	if onlyIfQueued {
		// no ack frame queuing, ackAlarm is zero or ackAlarm is not reached
		if !h.ackQueued && (h.ackAlarm.IsZero() || h.ackAlarm.After(now)) {
			return nil
		}
		// In here, means the timer is not zero and now have been exceeded the setting time
