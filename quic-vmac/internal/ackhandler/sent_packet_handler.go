...
func (h *sentPacketHandler) SentPacket(
	...
	// bytesSent influence the isAmplificationLimited(), which assure that sent packets could not larger than 3* received packets
	h.bytesSent += size
	...
	// largestSent is only used for judge receiving ack error (whether smaller than largestAck)
	pnSpace.largestSent = pn
	isAckEliciting := len(streamFrames) > 0 || len(frames) > 0

	// MOC: if it is the short header packet, we make it as non-ack eliciting packet
	...
	if isAckEliciting {
		...
		// bytesInFlight influence congestion control
		h.bytesInFlight += size
		if h.numProbesToSend > 0 {
			h.numProbesToSend--
		}
	}
	// actually, if isAckEliciting is false, which basically states that no congestion control is used
	h.congestion.OnPacketSent(t, h.bytesInFlight, pn, size, isAckEliciting)

	if !isAckEliciting {
		// non-ack eliciting packets will not be store as history
		p := getPacket()
		p.SendTime = t
		p.PacketNumber = pn
		...
		pnSpace.history.SentNonAckElicitingPacket(pn, p)
	...
	// the above packet information will be store as a history in pnSpace.history
...
func (h *sentPacketHandler) ReceivedAck(ack *wire.AckFrame, encLevel protocol.EncryptionLevel, rcvTime time.Time) (bool /* contained 1-RTT packet */, error) {
	...
	// MOC: although we remove ack, we make some rtt logic when receive a new stream frame
	//if encLevel == protocol.Encryption1RTT {
	//	return true, nil
	//}
	...
	if len(ackedPackets) > 0 {
		if p := ackedPackets[len(ackedPackets)-1]; p.PacketNumber == ack.LargestAcked() {
			...
			// TODO: MOC: UpdateRTT should be call when a client receive a new stream, because we remove ack so that we use other ways to estimate rtt
...
func (h *sentPacketHandler) setLossDetectionTimer() {
	// MOC: endpoints don't need to consider the loss, put it to seminar
	return
	...
	// PTO alarm (PTO is Persistent Timout, which normally occur in zero-window)
...
func (h *sentPacketHandler) OnLossDetectionTimeout() error {
	// TODO: MOC: Maybe OnLossDetectionTimeout also need to be baned
	h.logger.Errorf("IN LOSS DETECTION TIMEOUT, SHOULD NOT IN")
...
func (h *sentPacketHandler) SendMode(now time.Time) SendMode {
	...
	if numTrackedPackets >= protocol.MaxOutstandingSentPackets {
		if h.logger.Debug() {
			h.logger.Infof("Max outstanding limited: tracking %d packets, maximum: %d", numTrackedPackets, protocol.MaxOutstandingSentPackets)
		}
		return SendAck
	}
...
func (h *sentPacketHandler) queueFramesForRetransmission(p *packet) {
	...
	// MOC: not clear for next time request
	//p.StreamFrames = nil
	//p.Frames = nil
}
...
func (h *sentPacketHandler) RetransmitLoss(missingPacket protocol.PacketNumber) {
	index, ifExist := h.appDataPackets.history.getIndex(missingPacket)

	if ifExist {
		if len(h.appDataPackets.history.packets[index].Frames) != 0 || len(h.appDataPackets.history.packets[index].StreamFrames) != 0 {
			h.queueFramesForRetransmission(h.appDataPackets.history.packets[index])
		}
	} else {
		h.logger.Errorf("no find packet %d\n", missingPacket)
	}
}
