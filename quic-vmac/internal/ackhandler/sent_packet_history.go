...
func (h *sentPacketHistory) SentNonAckElicitingPacket(pn protocol.PacketNumber, p *packet) {
	h.checkSequentialPacketNumberUse(pn)
	h.highestPacketNumber = pn
	// if it is non-ack eliciting packet, we don't record the history data
	// MOC: non-ack eliciting packet need to be stored too
	//if len(h.packets) > 0 {
	if p != nil {
