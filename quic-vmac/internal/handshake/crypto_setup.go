...
func (h *cryptoSetup) StartHandshake() error {
	...
	for {
		// conn.Start() will start the handshake protocol. It may produce connection events, which may be read with NextEvent.
		ev := h.conn.NextEvent()
...
func (h *cryptoSetup) handshakeComplete() {
	...
	fmt.Println("Handshake Done.", h.handshakeCompleteTime)
	// call the complete function in connection.go, which just close the channel, but after close the channel
	// the main login run() will call the handleHandshakeComplete(), where is the accurate place to deal with handshake complete.
