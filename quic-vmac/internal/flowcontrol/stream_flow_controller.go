...
// AddBytesRead is called when receive_stream using Read() to read each frame
func (c *streamFlowController) AddBytesRead(n protocol.ByteCount) {
	c.mutex.Lock()
	c.baseFlowController.addBytesRead(n)
	// check if needed to send MAX_STREAM_DATA
...
func (c *streamFlowController) GetWindowUpdate() protocol.ByteCount {
	...
	if c.receiveWindowSize > oldWindowSize { // auto-tuning enlarged the window size
		c.logger.Infof("Increasing receive flow control window for stream %d to %d kB", c.streamID, c.receiveWindowSize/(1<<10))
