...
// baseFlowController is used to both connection-level and stream-level controlling
...
// offset is the Maximum Data field of these frames. So c.sendWindow denotes the maximum bytes can be sent in this stream or connection
func (c *baseFlowController) UpdateSendWindow(offset protocol.ByteCount) {
	if offset > c.sendWindow {
		//c.logger.Infof("update sendWindow from origin: %d to %d\n", c.sendWindow, offset)
		c.sendWindow = offset
	}
}

// UpdateSendWindowAuto is used by ICN-QUIC for eliminating the impact of lost MAX_{STREAM}_DATA frames
func (c *baseFlowController) UpdateSendWindowAuto() {
	c.sendWindow = c.sendWindow * 2
}

// sendWindowSize denotes the remaining size of data that can be sent
...
/* The above logic is related to a sender. The below logic is belongs to a receiver (what time to send a MAX_{STREAM}_DATA). */
