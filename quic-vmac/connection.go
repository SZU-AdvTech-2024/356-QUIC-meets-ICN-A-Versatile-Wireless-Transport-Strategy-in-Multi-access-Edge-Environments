...
var newConnection = func(
	...
	params := &wire.TransportParameters{
		...
		SeminarDataName:           "TestingDataName-SERVER",
	}
...
var newClientConnection = func(
	...
	params := &wire.TransportParameters{
		...
		ActiveConnectionIDLimit:   protocol.MaxActiveConnectionIDs,
		InitialSourceConnectionID: srcConnID,
		SeminarDataName:           "TestingDataName-CLIENT",
	}
	if s.config.EnableDatagrams {
		params.MaxDatagramFrameSize = protocol.MaxDatagramFrameSize
	} else {
...
func (s *connection) run() error {
	...
	if seminars[0].ifHandshakeDone {
		seminars[0].ifHandshakeDone = false
	}
	...
	if s.perspective == protocol.PerspectiveClient {
		...
		case zeroRTTParams := <-s.clientHelloWritten:
			// when the clientHello was received, client will send next crypto message (Handshake Finished)
			s.scheduleSending()
...
runLoop:
	for {
		...
		if !processedUndecryptablePacket {
			...
			case firstPacket := <-s.receivedPackets: // firstPacket here may denote the first of all packets in this round
		...
		if keepAliveTime := s.nextKeepAliveTime(); !keepAliveTime.IsZero() && !now.Before(keepAliveTime) {
			...
			s.logger.Infof("Sending a keep-alive PING to keep the connection alive.")
...
func (s *connection) HandshakeComplete() <-chan struct{} {
	// add current connection to the seminar
	seminars[0].connections = append(seminars[0].connections, s)
...
func (s *connection) handleHandshakeComplete() {
	...
	// after this function done, s.handshakeCtxCancel() will cancel the context and then pass this message to the server.go
	...
	// MOC: fix a rtt, the first params less, the congestion error occurs more frequent
	s.rttStats.UpdateRTT(180*time.Millisecond, protocol.DefaultMaxAckDelay, time.Time{})

	// Client handles transport parameters at here, while server handles it at the handshake process
	if s.perspective == protocol.PerspectiveClient {
	...
	// send Handshake DONE Frame and NewToken Frame
...
func (s *connection) handleHandshakeConfirmed() {
	...
	if !s.config.DisablePathMTUDiscovery && s.conn.capabilities().DF {
		...
		// MOC: ban the mtu discover process to fix the packet length
		// s.mtuDiscoverer.Start(utils.Min(maxPacketSize, protocol.MaxPacketBufferSize))
		//go func() {
		//	time.Sleep(3 * time.Second)
		//	ping, _ := s.mtuDiscoverer.GetPing()
		//	ping.Handler.OnAcked(nil)
		//}()
	}
}

func (s *connection) handlePacketImpl(rp receivedPacket) bool {
	// (before address validation) add bytes received and compare to bytes sent, check if we need start a loss detection timer
	...
	for len(data) > 0 {
		...
		if counter > 0 {
			...
			if destConnID != lastConnID {
				...
				s.logger.Infof("coalesced packet has different destination connection ID: %s, expected %s", destConnID, lastConnID)
				break
			}
		}
		...
		if wire.IsLongHeaderPacket(p.data[0]) {
			...
			// one shortHeader packet is handled only once
...
func (s *connection) handleShortHeaderPacket(p receivedPacket, destConnID protocol.ConnectionID) bool {
	...
	if s.logger.Debug() {
		...
		s.logger.Infof("<- Reading packet %d (%d bytes) 1-RTT", pn, p.Size())
		receivedPacketsNumber++
	}
	// main connection may receive duplicated packetNumber
	if s.receivedPacketHandler.IsPotentiallyDuplicate(pn, protocol.Encryption1RTT) {
		s.logger.Infof("Dropping (potentially) duplicate packet.")
		if s.tracer != nil {
...
func (s *connection) handleLongHeaderPacket(p receivedPacket, hdr *wire.Header) bool /* was the packet successfully processed */ {
	...
	s.logger.Infof("<- Reading packet %d (%d bytes), %s", packet.hdr.PacketNumber, p.Size(), packet.encryptionLevel)
...
func (s *connection) handleUnpackedShortHeaderPacket(
	...
	// MOC: 1-RTT packet, we remove the ack
	if isAckEliciting {
		isAckEliciting = false
	}

	// MOC: calculating the retransmission request interval
	seminars[0].marginTimeMutex.Lock()
	if rcvTime.Sub(seminars[0].lastReceiveTime) <= 2*time.Second {
		if !seminars[0].lastReceiveTime.IsZero() {
			// if smoothOneRoundTime is zero, means the current packet is the second
			if seminars[0].smoothOneRoundTime.Seconds() == 0 {
				seminars[0].smoothOneRoundTime = rcvTime.Sub(seminars[0].lastReceiveTime)
				// seminars[0].packetMarginTimeWriter.WriteString(seminars[0].smoothOneRoundTime.String() + "\n")
			} else {
				// 7/8*old + 1/8*new
				timeDiff := rcvTime.Sub(seminars[0].lastReceiveTime)
				seminars[0].smoothOneRoundTime = time.Duration((float32(seminars[0].smoothOneRoundTime/time.Microsecond)*oneRoundTimeMinusAlpha)+
					(float32(timeDiff/time.Microsecond)*oneRoundTimeAlpha)) * time.Microsecond
				// seminars[0].packetMarginTimeWriter.WriteString(seminars[0].smoothOneRoundTime.String() + "\n")
			}
		}
	}
	seminars[0].lastReceiveTime = rcvTime
	seminars[0].marginTimeMutex.Unlock()
...
func (s *connection) handleFrame(f wire.Frame, encLevel protocol.EncryptionLevel, destConnID protocol.ConnectionID) error {
	...
	case *wire.StreamFrame:
		if s.perspective == protocol.PerspectiveServer {
			if len(frame.Data) >= 9 && string(frame.Data[0:9]) == requestPrefix {
				missingOffsetByte := bytes.NewReader(frame.Data[9:])
				var missingOffset protocol.ByteCount
				binary.Read(missingOffsetByte, binary.LittleEndian, &missingOffset)
				seminars[0].offsetPnMap.RLock()
				if _, ok := seminars[0].offsetPnMap.m[missingOffset]; ok {
					fmt.Println("REQUEST>>>>>>>", missingOffset)
					seminars[0].connections[0].sentPacketHandler.RetransmitLoss(seminars[0].offsetPnMap.m[missingOffset])
				} else {
					fmt.Println("could not find the missing frame belonged to which packet")
				}
				seminars[0].offsetPnMap.RUnlock()
				return nil
			}
		}
		// if s.perspective == protocol.PerspectiveClient && !isBigFile {
		// 	if frame.Offset >= 570140 {
		// 		recordOnce.Do(func() {
		// 			streamEndTime = time.Now()
		// 			finalPacketsNumber = receivedPacketsNumber
		// 		})
		// 	}
		// } else {
		// 	if frame.Offset >= 7017000 {
		// 		recordOnce.Do(func() {
		// 			streamEndTime = time.Now()
		// 			finalPacketsNumber = receivedPacketsNumber
		// 		})
	...
	case *wire.MaxDataFrame:
		// MOC: if it is the MaxDataFrame, handled by the main connection
		// seminars[0].connections[0].handleMaxDataFrame(frame)
		s.handleMaxDataFrame(frame)
	case *wire.MaxStreamDataFrame:
		// MOC: if it is the MaxStreamDataFrame, handled by the main connection
		// err = seminars[0].connections[0].handleMaxStreamDataFrame(frame)
...
func (s *connection) handlePacket(p receivedPacket) {
	...
	default:
		s.logger.Errorf("protocol.MaxConnUnprocessedPackets Full......")
		if s.tracer != nil {
...
func (s *connection) handleStreamFrame(frame *wire.StreamFrame) error {
	...
	// stream frame is handled by receive_stream.go, it will update the stream flow controller and organize it in the stream frame queue
	return str.handleStreamFrame(frame)
}

// TODO: Notes that when setting the below two values, only one round sending can be run
func (s *connection) handleMaxDataFrame(frame *wire.MaxDataFrame) {
	// MOC: only set the maximum value to seminar's main stream; Notes that when setting the below two values, only one round sending can be run
	if frame.MaximumData >= seminars[0].maxReceivedData {
		seminars[0].maxReceivedData = frame.MaximumData
...
func (s *connection) handleMaxStreamDataFrame(frame *wire.MaxStreamDataFrame) error {
	...
	// MOC: only set the maximum value to seminar's main stream;
	if frame.MaximumStreamData >= seminars[0].maxReceivedStreamData {
		seminars[0].maxReceivedStreamData = frame.MaximumStreamData
...
func (s *connection) sendPackets(now time.Time) error {
	...
	if s.handshakeConfirmed && s.mtuDiscoverer != nil && s.mtuDiscoverer.ShouldSendProbe(now) {
		s.logger.Infof("From sendPackets in ShouldSendProbe")
		...
		// MOC: hack of receive ack for ping
		ping.Handler.OnAcked(nil)
		return nil
	}

	// MOC: every round sent packets, check if needed to send some control frames about flow control
	if isBlocked, offset := s.connFlowController.IsNewlyBlocked(); isBlocked {
		//s.connFlowController.UpdateSendWindowAuto()
		s.framer.QueueControlFrame(&wire.DataBlockedFrame{MaximumData: offset})
	}
	// send all the MAX_STREAM_DATA frames and MAX_DATA frame
...
var tCount = 0
var testBuffer *packetBuffer

// sendPacketsWithoutGSO is called rather than sendPacketsWithGSO in ICN-QUIC
func (s *connection) sendPacketsWithoutGSO(now time.Time) error {
	for {
		...
		//tCount++
		//if tCount == 8 || tCount == 29 {
		//	testBuffer = buf
		//	continue
		//}
		//if tCount == 15 {
		//	s.sendQueue.Send(testBuffer, testBuffer.Len())
		//}
		s.sendQueue.Send(buf, buf.Len())
...
func (s *connection) AcceptStream(ctx context.Context) (Stream, error) {
	...
	// when error no happened and is server
	if err == nil && s.perspective == protocol.PerspectiveServer && !seminars[0].ifSetMainStream {
		fmt.Println("Accept a new Stream and set the main stream: ", str.StreamID())
		// setMainStream() should be called when the server aggregate all the clients request
		str.SetMainStream()

		// we only restore the sending stream now
		seminars[0].mainStream = str
		seminars[0].ifSetMainStream = true
		//go func() {
		//	time.Sleep(3 * time.Second)
		//	seminars[0].mainStream.SetMainStream()
		//	seminars[0].mainStream.Write(IQDataToBeSent)
		//}()
	}
...
func (s *connection) OpenStreamSync(ctx context.Context) (Stream, error) {
	// MOC: record the stream to send retransmission request later
	str, err := s.streamsMap.OpenStreamSync(ctx)
	if err == nil && s.perspective == protocol.PerspectiveClient {
		seminars[0].mainStream = str
	}
	return str, err
}
...
// newFlowController is the stream flow controller. connection flow controller is initialed at preSetup
...
// onHasStreamWindowUpdate queues a MAX_STREAM_DATA frame to control queue
func (s *connection) onHasStreamWindowUpdate(id protocol.StreamID) {
	s.windowUpdateQueue.AddStream(id)
	s.scheduleSending()
}

// onHasConnectionWindowUpdate queues a MAX_DATA frame to control queue
...
func (s *connection) onHasStreamData(id protocol.StreamID) {
	// push the current stream that have data into the streamQueue
