...
// handlePacketImpl is where quic handles the first packet of a connection (initial packet)
...
func (s *baseServer) handleInitialImpl(p receivedPacket, hdr *wire.Header) error {
	...
	// here we will generate the first connectionID, the other 3 connectionIDs are created in connection.go/applyTransportParameters() ,
	// which is used to set the maximum number of connectionID according to the active_connection_id_limit transport parameter
	...
	if added := s.connHandler.AddWithConnID(hdr.DestConnectionID, connID, func() (packetHandler, bool) {
		...
		// just put the first packet into the channel of this connection. It will be handled later
		conn.handlePacket(p)
	...
	go conn.run()
	// wait until the connection accepted
...
func (s *baseServer) handleNewConn(conn quicConn) {
	...
	if s.acceptEarlyConns {
		...
		case <-conn.HandshakeComplete():
			// FIXME: Normally, the name of seminar should be obtained from s.peerParams
			// We create Seminar initially, because currently the interface quicConn is different with struct connection
			// So the below logic is shifted to connection.go/HandshakeComplete()
			//s.createOrUpdateSeminar("CHAT_GPT")
		case <-connCtx.Done():
			return
		}
	}
...
func (s *baseServer) createOrUpdateSeminar(dataName string) {
	s.logger.Infof("Server Knows Handshake Completed, check if needed create seminar")
	if len(seminars) == 0 {
		seminars = append(seminars, *newSeminar(dataName))
	} else if dataName != BaseDataName {
		s.logger.Errorf("Could not handle different dataName now.")
	}
}
